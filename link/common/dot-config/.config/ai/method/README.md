...
| `guidelines [--language <L>]... [--caller <p>] [--section FILE:HEADING]... [--dom] [--state]` | The required reading for those languages as text: short files whole, long ones cut to the sections a run must have loaded, plus any section named outright with `--section`, and a "Not loaded" report for anything a reorganised guideline no longer satisfies | 0 · **2** no guidelines dir |
