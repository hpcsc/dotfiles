### 3. Match the branch against the request
...
Record it: `clerk step --done match-request`, with `--mismatch "<the story's words>"` once per mismatch. A recorded mismatch parks the run until the user decides; then `clerk step --done match-request --resolved`.
