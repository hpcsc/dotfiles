...
The behavioral question stays decisive. Where it is unclear, the producer question sharpens it: **can whatever records the fact at this stage even see the distinction?** If a provider callback reports delivery and knows nothing about the message's intent, nothing upstream is capable of making the delivered variants differ, so the distinction belongs to an earlier stage. That question you can settle by looking; "will these ever diverge?" you cannot.

Treat the producer question as a tiebreaker, never as a substitute. A producer blind to the distinction does **not** mean no consumer needs it — a consumer can recover it from the send's identity and branch anyway, which is the same mechanism that keeps intent derivable after merging. Marketing and transactional email are reported by one provider that cannot tell them apart, yet they drive entirely different downstream work; there the producer question says merge and the behavioral question correctly overrules it. Split when *either* question says split. See [external-communications.md](external-communications.md) for the worked example, for the per-send keying that merging depends on, and for why one type is the cheaper default when the answer stays unclear.
