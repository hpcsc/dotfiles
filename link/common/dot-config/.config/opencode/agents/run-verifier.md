...
```json
{
  "clean": true,
  "findings": [
    { "check": "staged-tail|vacuous-receipt|dead-code|commit-boundary", "severity": "block|warn", "detail": "<file/symbol/commit + the concrete problem + the fix>" }
  ],
  "learnings_path": "<path or null>"
}
```

`clean` is true only when there are **no `block` findings** (warns are allowed). Order findings most-severe first.

## What You Must NOT Do

- Modify, stage, or commit any files — remediation is the caller's job
- Return anything other than the JSON verdict above
