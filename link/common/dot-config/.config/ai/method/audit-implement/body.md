...
*distinct* runtime or test finding, one reader for up to eight quality findings settled by
reading, and one report. A typical branch lands around 10–20 agents and 15–25
...
4. **Refute** — every candidate finding goes to an independent agent instructed to **refute** it. A **runtime** claim must be reproduced by execution — a failing test, a `-race` run, a benchmark, a direct invocation — and the raw output is the evidence; unreproduced means dropped. A **quality** claim is not dischargeable by execution, so it is checked differently rather than discarded: the refuter must find the rule (guideline, CLAUDE.md, or consistent surrounding practice) and cite the specific line. Quality claims like that change nothing and need no checkout, so one reader takes up to eight of them, grouped by file, and returns a verdict for each; a claim it returns no verdict for is kept as not executed, never dropped. A vacuity claim about a test *is* checkable — break what the test names and see whether it still passes — so it keeps a refuter and a checkout of its own.

5. **Report** — survivors are **re-graded against one severity rubric** and then ranked, with their evidence and a `confidence` of `confirmed` (reproduced, or rule cited at a line) or `plausible`. The report agent writes only the grades, the ranking and the gaps; clerk joins each finding's claim, file, lens and evidence onto them, so a survivor the agent leaves out is still reported and an id it invents is dropped. The re-grade is not cosmetic: each lens grades alone and the scales drift apart, so `low` has meant both "a doc comment does not open with the symbol name" and "a live DynamoDB read per mailbox on every request". This is the only stage that sees the whole panel at once, so it is the only place the grades can be made comparable — severity is graded on what the defect costs, not on how central it is to the lens that found it. Refuted candidates come back separately with *why* they were dropped, so you can disagree. And `coverage_gaps` names what the audit could **not** judge — a lens that did not run, a changed file no language claimed, a claim nobody could test.
