...
- **Prefer a single structural assertion** — When asserting on a composite value (struct, slice, map), use one `require.Equal` against the full expected value rather than multiple `require.Equal` / `require.Len` calls on individual fields or elements. One assertion covers shape, length, and contents together, and the diff on failure shows everything at once.
