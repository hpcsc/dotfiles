...
Record it: `clerk step --done match-request`, with `--mismatch "<the request's words>"` once per mismatch. A recorded mismatch parks the run until the user decides; then `clerk step --done match-request --resolved`.
