...
It puts the tree under `.worktrees/` beside the git dir, branches from HEAD (`--base <ref>` to branch from elsewhere), and adds `.worktrees/` to the repo's `info/exclude` so the directory it just made does not read as a dirty tree to the next `clerk prepare`. It reports the `path`, and whether it `created` the worktree or `adopted` one that was already there.
...
**Pass `path`, never `name`.** `name` is the tool's own creation mode and it puts the worktree under `.claude/worktrees/`, which is compiled in and takes no setting — so `name` is simply unable to honour the location this method uses. Creating it with `clerk` first and entering by path is what makes the two harnesses put the work in the same place. The tool accepts any path registered in `git worktree list` for this repo when you enter from the launch directory, which is where this phase runs.
