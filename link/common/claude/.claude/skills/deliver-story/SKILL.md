...
- **One learnings file, the repo's own.** The driver names no learnings path, so every deliverable writes the file `clerk prepare` resolves: `tasks/learnings.md` in the main checkout, which is what the run planning the next story reads. It used to hand each deliverable its own file under `~/.claude/implement-learnings/` to keep parallel appends apart; that bought isolation at the price of a store nothing ever read back, and the wave's learnings never reached the next wave. The appends are `O_APPEND` and land whole, so a wave finishing together is safe; what a race can still cost is `clerk learn`'s duplicate-title check, which reads before it writes.
