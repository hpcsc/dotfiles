...
- **Constant pin** — the expected value is a literal copied from the production source, and the code performs no transformation to produce it. `require.Equal(t, 1, schemaVersion)` where the code hardcodes `schemaVersion = 1`: any stub returning `1` passes. (This is the *Weak Independence* change detector, seen from the code's side.)
...
//   require.Equal(t, "ReviewCompleted", event.ReviewCompleted{}.EventName())
// Encode's own logic (the registry lookup) is never exercised.
typeName, _, _, _ := event.Encode(event.ReviewCompleted{})
require.Equal(t, "ReviewCompleted", typeName)
```
...
- A **golden / contract test** over frozen external input — `Decode(frozenBytes)` stubbed to a constant no longer reproduces the expected struct, so it fails. The frozen input the code must interpret is what makes it real.
...
| Assertion | Caller depends on it? | Verdict |
|---|---|---|
| `Withdraw` returns `ErrInsufficientFunds` | Yes — callers handle this error | **Behavior** |
...
|---|---|
| `require.True(t, mockValidator.called)` | `require.EqualError(t, err, "amount must be positive")` |
| `require.Equal(t, 1, spy.callCount)` | `require.Equal(t, "cust-123", recorder.lastRecipient)` |
...
require.Equal(t, []string{"validate", "persist"}, spy.callOrder)
```
...
// Behavioral — the second call is faster (observable performance contract).
// But better: test the actual behavior (cache hit returns same result).
result1, _ := service.GetUser("123")
result2, _ := service.GetUser("123")
