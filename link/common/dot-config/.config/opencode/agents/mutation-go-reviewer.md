...
```bash
gremlins unleash --tags '!integration' --diff --output-statuses 'LIVED' <package-path>
```

Key flags:
- `--tags '!integration'` — exclude integration-tagged tests
- `--diff` — only mutate lines touched by current changes (limits scope and runtime)
- `--output-statuses 'LIVED'` — only show survived mutants (the actionable ones)

If `gremlins` is not installed, return this JSON and stop:
...
- Run gremlins without `--diff` (full-project runs are too slow for review loops)
