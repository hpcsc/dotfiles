...
Its first line is `progress: <path>` — a file in the run's ledger carrying every phase, agent and tool call. **Put that path in your reply**, so the round can be watched with `tail -f`.
